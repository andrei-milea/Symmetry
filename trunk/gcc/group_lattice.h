	...
	void CyclicExtensionMethod(G &group)
	{
		...
		for(std::size_t index = 0; index < m_Lattice.size(); index++)	
		{
					...
					subgrp_iter != GetLayer(index).end(); subgrp_iter++)
			{
				...
				std::vector<T> norm_elem = m_Group.GetNormalizerEl(*index);
				...
				for(std::vector<T>::iterator elem_iter = possible_gen.begin();
						elem_iter != possible_gen.end(); elem_iter++)
				{
					if(Has_prime_pow_el(*iter, *elem_iter))
					{
...
private:
	...
	void BuildCycSubgrpPPOrder()
	{
		...
		{
			vector<T> cyc_subgrp_elem = m_Group.GetCyclicSubgroup(*iter);
