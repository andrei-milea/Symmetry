...
public:
	...
	*/
	std::vector<T> GetElementsDimino()const
	{
		...
		std::vector<T> elements = GetCyclicSubgroupEl(*m_GenSet.begin());
	...
	*/
	...
	/*!
	  returns the cyclic subgroup elements generated by the given element
	  by computing it's powers until identity is reached
	  Complexity: O(n), where n is the order of the element
	*/
	std::vector<T> GetCyclicSubgroupEl(const T& element)const
	{
		T temp_el = element;
	...
	/*!
	  returns the cyclic subgroup $C_n$ elements by rotating the identity
	  permutation of the given size
	  Complexity $O(n)$, where n is the size given as parameter
	*/
	std::vector<T> GetCyclicSubgroupEl(const std::size_t size)
	{
		std::vector<T> cyclic_group;
		T elem(size);
