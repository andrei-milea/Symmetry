...
void cGetInverseCommand::Execute()
{
	if((1 != m_Expression.terms.size()) || (1 != m_Expression.terms[0].factors.size()))
		throw std::runtime_error(CONTEXT_STR + " Invalid input. Provide a valid square matrix to compute the inverse.");
	...
	m_Result.resize(cols_no, cols_no);
	bool res = get_inverse(input_matrix, m_Result);
	if(false == res)
		throw std::runtime_error(CONTEXT_STR + " Invalid input. Matrix not invertible.");
};
