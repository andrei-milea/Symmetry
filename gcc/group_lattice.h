...
public:
	...
	void CyclicExtensionMethod()
	{
		...
		for(std::size_t index = 0; /*index < m_Lattice.size()*/ index < 2; index++)	
		{
			need_new_layer = true;
					...
					subgrp_iter != GetLayer(index).end(); subgrp_iter++)
			{
				...
				std::vector<T> possible_gen = std_ex::set_intersection(m_CycSubgrpsGen, 
						norm_elem);
				...
				while(!possible_gen.empty())
				{
					...
					if(Has_prime_pow_el(*subgrp_iter, generator))
					{
						...
						std_ex::print_vector(subgrp.GetElementsDimino());
						std_ex::print_vector(possible_gen);
